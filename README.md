<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1e3a8a,50:6d28d9,100:be185d&height=220&section=header&text=Zakaria%20Chamekh&fontSize=48&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=D%C3%A9veloppeur%20Web%20Full%20Stack%20Junior&descAlignY=58&descSize=18" width="100%"/>

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=A855F7&center=true&vCenter=true&width=600&lines=React.js+%7C+Laravel+%7C+MySQL;D%C3%A9veloppeur+Full+Stack+Junior;Passionn%C3%A9+par+le+Web+%26+les+API+REST;Toujours+pr%C3%AAt+%C3%A0+apprendre+%F0%9F%9A%80" alt="Typing SVG" />

<br/>

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](#)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:chamekhzakaria95@gmail.com)

</div>

---

### 👋 À propos de moi

- 🎓 Licence en Développement Logiciel & Sécurité des Systèmes — **ENSAM Casablanca** (2025–2026)
- 💻 Diplôme en Développement Digital, Option Web Full Stack — **ISFO Casablanca**
- 📍 Basé à **Casablanca – Sidi Maarouf, Maroc**
- 🛠️ Maîtrise **React.js**, **Laravel (MVC)**, **MySQL**, **API RESTful**
- 🔐 Expérience en authentification sécurisée (JWT, OAuth, OTP, Magic Link)
- 🌱 En apprentissage continu : **Spring Boot**, **Node.js**, **Docker**
- ⚡ Recherche un poste **Full Stack** pour contribuer à des projets concrets

---

### 🧪 En ce moment

- 🚀 Développement de **Electro-05**, une plateforme e-commerce multi-vendeurs (React + Laravel + Redis + Docker)
- 📈 Approfondissement de mes compétences en architecture headless et API sécurisées
- 🧩 Exploration de Spring Boot et Power BI

---

### 🛠️ Stack Technique

<div align="center">

**Langages**

<img src="https://skillicons.dev/icons?i=html,css,js,php,java,mysql" />

<br/><br/>

**Frameworks & Librairies**

<img src="https://skillicons.dev/icons?i=react,laravel,tailwind,spring,nodejs" />

<br/><br/>

**Base de données & DevOps**

<img src="https://skillicons.dev/icons?i=mysql,mongodb,docker,git,github,linux,postman" />

</div>

---

### 📊 Statistiques GitHub

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=TON_USERNAME&show_icons=true&theme=radical&hide_border=true&bg_color=0d1117&title_color=A855F7&icon_color=EC4899" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=TON_USERNAME&layout=compact&theme=radical&hide_border=true&bg_color=0d1117&title_color=A855F7" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=TON_USERNAME&theme=radical&hide_border=true&background=0D1117&ring=A855F7&fire=EC4899" />

</div>

---

### 📈 Graphique d'activité

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=TON_USERNAME&theme=react-dark&hide_border=true&bg_color=0D1117&color=A855F7&line=EC4899&point=ffffff" width="100%"/>

</div>

---

### 🐍 Contribution Snake (animation 3D)

<div align="center">

<img src="https://raw.githubusercontent.com/TON_USERNAME/TON_USERNAME/output/github-contribution-grid-snake-dark.svg" width="100%"/>

</div>

> 💡 Pour activer le snake animé : ajoute le fichier `.github/workflows/snake.yml` (voir instructions en bas) — il génère automatiquement ce SVG à partir de ton graphique de contributions.

---

### 🎓 Formation

| Diplôme | École | Période |
|---|---|---|
| Licence Développement Logiciel & Sécurité des Systèmes | ENSAM – Casablanca | 2025 – 2026 |
| Diplôme Développement Digital – Web Full Stack | ISFO – Casablanca | 2023 – 2025 |
| Baccalauréat Sciences Physiques et Chimiques | Lycée Ibnou Arabi – Casablanca | 2022 – 2023 |

---

### 💼 Expérience

**Stagiaire Développeur WordPress — Sparkvision** *(Mars 2025, à distance)*
- Développement complet d'un site vitrine WordPress avec thème personnalisé et responsive design
- Optimisation SEO on-page (balises meta, structure Hn, mots-clés)
- Collaboration Agile à distance avec l'équipe marketing

---

### 🚀 Projet phare — Electro-05 (E-commerce Full Stack)

`React.js` `Laravel 12` `MySQL` `Redis` `Docker` `API REST` `Tailwind CSS`

- Plateforme e-commerce multi-vendeurs en architecture Headless
- Modélisation UML + base MySQL relationnelle (20+ tables)
- API REST sécurisées (Laravel Sanctum, Email/Password, OTP, Magic Link, OAuth Google)
- Fonctionnalités : catalogue, panier, commandes, wishlist, avis clients, factures PDF
- Interface React responsive, multilingue, Context API
- Optimisation Redis, conteneurisation Docker Compose, tests Postman

---

### 🏆 Certifications

- ✅ Web Development Fundamentals — IBM SkillsBuild
- ✅ Développement Moderne JavaScript — Cisco
- ✅ CSS Essentials — Cisco

---

### 🌍 Langues

`Arabe : Courant` &nbsp;•&nbsp; `Français : Intermédiaire` &nbsp;•&nbsp; `Anglais : Notions`

---

<div align="center">

### 📫 Me contacter

📧 chamekhzakaria95@gmail.com &nbsp;|&nbsp; 📱 +212 704 685 662

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:be185d,50:6d28d9,100:1e3a8a&height=120&section=footer" width="100%"/>

</div>
